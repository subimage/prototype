...
suite("String Namspace",function(){
  ...
  test(".gsub() With Replacement String",function() {
    ...
    assert.equal('foobooboz',source.gsub(/\s+/, ''));
    assert.equal('  z',source.gsub(/(.)(o+)/, ''));
  ...
  test(".scan()",function() {
    ...
    assertenum([2, 2, 1], results);
    assert.equal(source, source.scan(/x/, fail));
    assert(typeof str == 'string');
  });
  
  test(".toArray()",function() {
    assertenum([],''.toArray());
    assertenum(['a'],'a'.toArray());
    assertenum(['a','b'],'ab'.toArray());
    assertenum(['f','o','o'],'foo'.toArray());
  });
  ...
  */
  ...
  test(".extractScripts()", function() {
    assertenum([], 'foo bar'.extractScripts());
    assertenum(['boo();'], ('foo <script>boo();<'+'/script>bar').extractScripts());
    assertenum(['boo();','boo();\nmoo();'], 
  ...
  test(".escapeHTML()", function() {
    ...
    benchmark(function() { largeTextUnescaped.escapeHTML() }, 1000);
  });
  ...
  test(".unescapeHTML", function() {
    ...
    benchmark(function() { largeTextEscaped.unescapeHTML() }, 1000);
    
  });
  
  test("Template Evaluation", function() {
  ...
  test("Template Evaluation With Empty Replacement", function() {
  ...
  test("Template Evaluation With Falses",function() {
  ...
  test("Template Evaluation With Nested", function() {
  ...
  test("Template Evaluation With Indexing", function() {
  ...
  test("Template To Template Replacements", function() {
  ...
  test("Template Evaluation Combined",function() {
  ...
  test(".toQueryParams()", function() {
    ...
    assertHashEqual({a:'  '}, 'a=++'.toQueryParams(), 'value of spaces');
    ...
    assertenum($w('r g b'), 'col=r&col=g&col=b'.toQueryParams()['col'],
      'collection without square brackets');
    var msg = 'empty values inside collection';
    assertenum(['r', '', 'b'], 'c=r&c=&c=b'.toQueryParams()['c'], msg);
    assertenum(['', 'blue'],   'c=&c=blue'.toQueryParams()['c'], msg);
    assertenum(['blue', ''],   'c=blue&c='.toQueryParams()['c'], msg);
  });
  ...
  test(".evalJSON()", function() {
    ...
    assert.equal(valid.evalJSON().test,'hello world!');
    assert.equal(valid.evalJSON(true).test,'hello world!');
    assert.throws(function() { invalid.evalJSON() },SyntaxError);
    assert.throws(function() { invalid.evalJSON(true) },SyntaxError);

    attackTarget = "Not scared!";
    assert.throws(function(){dangerous.evalJSON(true)},SyntaxError);
    assert.equal("Not scared!", attackTarget);

    assert.equal(('/*-secure- \r  \n ' + valid + ' \n  */').evalJSON().test,'hello world!');
    var temp = Prototype.JSONFilter;
    Prototype.JSONFilter = /^\/\*([\s\S]*)\*\/$/; // test custom delimiters.
    assert.equal(('/*' + valid + '*/').evalJSON().test,'hello world!');
    Prototype.JSONFilter = temp;
    
    assert.strictEqual(huge.evalJSON(true).last().test,123);
